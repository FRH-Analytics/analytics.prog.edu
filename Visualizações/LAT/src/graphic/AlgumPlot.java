...
public class AlgumPlot {
	...
	private void loadData() {
		...
		System.out.println(qtdEixoX + "," + qtdEixoY);
	...
	public void drawSquares() {
		for (int i = 0; i < data.length; i++) {
			for (int j = 0; j < data[i].length; j++) {
				parent.fill(255 - ((float) data[i][j][QTD] / 100) * 255);
				parent.stroke(255 - ((float) data[i][j][QTD] / 100) * 255);
				parent.rect(startAtX + squareSpace + i * squareSpace, startAtY + (data[i].length + 1 - j) * squareSpace, squareSpace, squareSpace);
			}
		}		
	}
	...
	public void drawXAxis() {
		for (int i = 0; i < data.length; i++) {
//			parent.text("" + (data[i][0][X_CORD] + 1), startAtX + (i + 1) * squareSpace, startAtY + squareSpace / 2);
			parent.text("" + (data[i][0][X_CORD] + 1), startAtX + (i + 1) * squareSpace, startAtY + (data[i].length + 3) * squareSpace);
		}
	}
...
//			parent.text("" + (data[0][i][Y_CORD] + 1), startAtX, + (i + 1) * squareSpace, startAtY + squareSpace / 2);
			parent.text("" + (data[i].length - data[0][i][Y_CORD]), startAtX, startAtY + (i + 3) * squareSpace);
		}
	}
...
//		for (int i = 0; i < data.length; i++)
//			if (mouse.x >= startAtX + squareSpace + (data[i]) * squareSpace && mouse.x <= startAtX + squareSpace + (this.x[i]) * squareSpace + squareSpace)
...
//	}
	...
	public int[] getSquare(PVector mouse) {
		for (int i = 0; i < data.length; i++)
			if (mouse.x >= startAtX + squareSpace + i * squareSpace && mouse.x <= startAtX + squareSpace + i * squareSpace + squareSpace)
				for (int j = 0; j < data[i].length; j++)
					if (mouse.y >= startAtY + (data[i].length + 1 - j) * squareSpace && mouse.y <= startAtY + (data[i].length + 1 - j) * squareSpace + squareSpace)
						return new int[] {data[i][j][X_CORD] + 1, data[i][j][Y_CORD] + 1, data[i][j][QTD]};
		return null;
	}
	
	public float[] getSquare2(PVector mouse) {
